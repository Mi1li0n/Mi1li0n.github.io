...
test('ore can be equipped through the real inventory action and survives migration',()=>{
 const g=game();assert.equal(g.act({type:'equip',slot:1,item:'ore'}),true);assert.equal(g.state.player.hands[1],'ore');assert.equal(g.state.turn,1);g.load(1);assert.equal(g.state.player.hands[1],'ore');g.setAmount('ore',0);assert.equal(g.act({type:'equip',slot:1,item:'ore'}),false);
});
test('the three chapter route transitions still lead to victory with equipment and allies',()=>{
 const g=game(),s=g.state;s.player.gear.helmet=true;s.allies.push({name:'埃文',role:0,used:true});
 for(let n=0;n<3;n++){assert.equal(s.level,n);s.mode='playing';s.monsters=[];const exit=s.sites.find(a=>a.id==='water');exit.discovered=true;s.player.x=exit.x;s.player.y=exit.y;s.panel=null;assert.equal(g.act({type:'interact'}),true);assert.equal(g.act({type:'choose',kind:'depart'}),true);assert.equal(s.player.gear.helmet,true);assert.ok(s.allies.some(a=>a.name==='埃文'));}
 assert.equal(s.mode,'won');assert.equal(s.routeHistory.length,3);
});
test('priority enemy attacks still resolve before throwing ore and cancel use on death',()=>{
 const g=game();arena(g);const s=g.state;s.player.hands=['pick','ore'];s.player.hp=1;s.player.armor=0;const m=g.createMonster('undead',31,30,0,[-1,0]);s.monsters=[m];g.planMelee(m);const before=s.ore;g.act({type:'use',item:'ore'});assert.equal(s.mode,'dead');assert.equal(s.ore,before);assert.equal(s.turn,1);
});
